...
import { loginAPI, checkAuthStatusAPI } from "../../APIServices/users/usersAPI";
import AlertMessage from "../Alert/AlertMessage";
...
const Login = () => {
  ...
  // useEffect to handle data from auth status check
  useEffect(() => {
    if (isSuccess && data) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      userMutation
        ...
        .then(() => {
          navigate("/dashboard");
          refetch(); // trigger the checkAuthStatusAPI after successful login
        })
        .catch((err) => console.log(err));
    },
  });
      ...
      <div className="w-full p-4">
        <div className="flex flex-col justify-center py-24 max-w-md mx-auto h-full">
          <form onSubmit={formik.handleSubmit}>
            ...
            <a
              href="http://localhost:8080/api/v1/users/auth/google"
              className="h-14 inline-flex items-center justify-center gap-2 py-4 px-6 rounded-full bg-white w-full text-center border border-gray-100 shadow hover:bg-gray-50 focus:ring focus:ring-orange-200 transition duration-200"
              type="submit"
            >
              <svg
